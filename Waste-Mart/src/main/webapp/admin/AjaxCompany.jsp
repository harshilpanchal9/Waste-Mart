...
<%
	...
	if(action!=null){
		...
		if(status.equalsIgnoreCase("active")){
			qry = "select count(*) as totalconsumers from company_signup where status='Active'";
		}	
		else if(status.equalsIgnoreCase("deactive")){
			qry = "select count(*) as totalconsumers from company_signup where status='Deactive'";
		}
		else{
			temp = "All";
			qry = "select count(*) as totalconsumers from company_signup where status in ('Active','Deactive')"; 
		}
        if(search.length()>0){
			qry = qry + " and companyname like '%"+search+"%'";
		}		
	...
	if(status.equalsIgnoreCase("active")){
		qry = "select * from company_signup where status='Active'";
	}	
	else if(status.equalsIgnoreCase("deactive")){
		qry = "select * from company_signup where status='Deactive'";
	}
	else{
		temp = "All";
		qry = "select * from company_signup where status in ('Active','Deactive')"; 
	}
	if(search.length()>0){
		qry = qry + " and companyname like '%"+search+"%'";
	}
    ...
    if(!rs.first())
    {
    	...
    	out.println("</script>");
    	out.println("No any company user to view");
    	out.println("</div>");
    	out.println("</li>");    	 
    }
    rs.relative(-1);
   	...
   	{       
    	...
    	out.println("</script>");
   		...
   		out.println("<a href='#'>");
   		out.println("<div class='item-img rounded' style='background-image: url(../company-images/"+rs.getString(8)+")'></div>");
   		out.println("</a>");
   		out.println("</div>");
   		...
   		out.println("<div class='no-overflow' style='font-weight: 600;'> "+ rs.getString(2)+" </div>");
   		out.println("</div>");
   		...
   		out.println("<div>");
   		out.println("<a href='mailto:"+rs.getString(4)+"'>");
   		out.println("<h3 class='item-title'>"+rs.getString(4)+"</h3>");
   		out.println("</a>");
   		out.println("</div>");
   		out.println("</div>");
   		...
   		out.println("<div class='no-overflow'>"+ rs.getString(3)+"</div>");
   		out.println("</div>");
   		out.println("<div name='divdate' class='item-col item-col-stats no-overflow' style='text-align: center;flex-grow: 1.9;padding-right: 5px;'>");                                        
                ...
                String r_date="",l_date="",datetime=rs.getString(6);
            ...
            out.println("<div name='divlogindate' class='item-col item-col-date no-overflow' style='text-align: center;flex-grow: 1.9;padding-right: 5px;'>");
                ...
                out.println("<div class='no-overflow'>");
                String lastlogin=rs.getString(7);
            ...
            out.println("<div class='item-col item-col-date' style='text-align: center;flex-grow: 1.5;padding-top: 16px;justify-content: center;'>");
                ...
                out.println("<div class='no-overflow'>");
                out.println("<form action='../UserCompanyController'>");                	                
               	out.println("<input type='hidden' name='company_id' value='"+rs.getInt(1)+"'><input type='hidden' name='status' value='"+rs.getString(9)+"'><input type='hidden' name='all' value='"+temp+"'>");
	            // Check the user inactivity
	           		...
	           		if(checkStatus.equalsIgnoreCase("Active")){
	               		...
	               		if(d1.before(d2)){
	               			String qry1 = "update company_signup set status=? where companyid=?";
               	...
               	String statusname = rs.getString(9);               	
               		if(statusname.equalsIgnoreCase("active")){						
               			out.println("<button type='submit' name='btn' value='CompanyStatus' class='btn btn-oval btn-success' style='color:#ffffff;background-color: #4bcf99;border-color: #4bcf99;color:#ffffff;box-shadow:none;outline-offset:none;'>"+rs.getString(9)+"</button>");
               		}else{               			
               			out.println("<button type='submit' name='btn' value='CompanyStatus' class='btn btn-oval btn-success' style='color:#ffffff;background-color: #ff0000;border-color: #ff0c00;color:#ffffff;box-shadow:none;outline-offset:none;'>"+rs.getString(9)+"</button>");
               		}	                	                	
            	out.println("</form>");
            	out.println("</div>");
            out.println("</div>");
                ...
                out.println("<div class='item-actions-dropdown'>");
                    ...
                    out.println("<div class='item-actions-block' style='height: 37px;top: -4px;line-height: 36px;border-radius: 15px;'>");
                        out.println("<ul class='item-actions-list'>");
                            ...
                            out.println("<a class='edit' href='viewcompany.jsp?companyid="+rs.getInt(1)+"'>");
