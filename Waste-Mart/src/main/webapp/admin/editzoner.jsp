...
<head>
        ...
        <title> Admin | Zoner Edit Profile </title>
                ...
                <article class="content items-list-page">
                    <div class="title-search-block">
                        <div class="title-block">
                            <h3 class="title"> Zoner Update Profile                                      
               		...
               		<section class="section">
                    	...
                    	System.out.println("Custid: "+zonerid);                    
                                    ...
                                    <form method="post" action="../UserZonerController" class="signin-form" onsubmit="return submitValidation()" enctype="multipart/form-data">
                                    	<div class="container">
			    							<div class="avatar-upload">
			        							<div class="avatar-edit">
			        								<input type="hidden" name="oldimage" value="<%=rs.getString(11)%>">
			           									<input type="file" name="photo" size="50" id="imageUpload" accept=".png, .jpg, .jpeg" />
			         		   							<label for="imageUpload"></label>
			       	 							</div>
			        							<div class="avatar-preview">
			            							<div id="imagePreview" style="background-image: url(../zoner-images/<%=rs.getString(11)%>);"></div>
			        							</div>
			    							</div>
										</div>
										<input type="hidden" name="zonerid" value="<%=rs.getInt(1) %>"/>
                                     	<div id="divfname" class="form-group">
                                            ...
                                            <input type="text" id="fname" class="form-control underlined" name="firstname" value="<%=rs.getString(2) %>" onkeyup="firstNameValidation()">
                                        	<span class="has-error" id="fnameerror" style="visibility:hidden"></span>
                                        </div>
                                        <div id="divlname" class="form-group">
                                            <label for="exampleInputEmail1">Last name</label>
                                            <input type="text" id="lname" class="form-control underlined" name="lastname" value="<%=rs.getString(3) %>" onkeyup="lastNameValidation()" >
                                            <span class="has-error" id="lnameerror" style="visibility:hidden"></span>
                                        </div>
                                         <div id="divaddress" class="form-group">
                                            <label for="exampleInputEmail1">Address</label>
                                            <textarea id="address" rows="3" class="form-control underlined" name="address" onkeyup="removevalidation()"><%=rs.getString(4) %></textarea>
                                            <span class="has-error" id="addresserror" style="visibility:hidden"></span>
                                        </div>
                                         <div id="divphone" class="form-group">
                                            <label for="exampleInputEmail1">Phone Number</label>
                                            <input type="text" id="phone" class="form-control underlined" name="phoneno" value="<%=rs.getString(5) %>" onkeyup="phoneValidation()">
                                            <span class="has-error" id="phoneerror" style="visibility:hidden"></span>
                                        </div>
                                        <div id="divemail" class="form-group">
                                            <label for="exampleInputEmail1">Email address</label>
                                            <input type="email" id="email" class="form-control underlined" name="email" value="<%=rs.getString(6) %>" onkeyup="emailValidation()">
                                        ...
                                        <div class="form-group has-error">
                                            ...
                                            <div class="form-group has-error">
                                            	...
                                            	<%
                                            		String zonename=rs.getString(7);
                                            		int flag=0;
                                            		String getZone="select * from zone";
                                            		PreparedStatement st2=cn.prepareStatement(getZone);
                                            		ResultSet rs2=st2.executeQuery();
                                            		while(rs2.next())
                                            		{
                                            			if(zonename.equalsIgnoreCase(rs2.getString(2)))
                                            			{
                                            				flag=1;
                                            			}
                                            			if(flag==1)
                                            			{
                                            	%>
                                            		<option selected value="<%=rs2.getString(1) %>"><%=rs2.getString(2)%></option>
                                            	<%
                                            		flag=0;
                                            			}
                                            			else
                                            ...
                                            <label>Select Zone</label>
                                            <div class="form-group has-error">
                                            	<select name="zoneareaid" id="zonearea" class="form-control" id="dropdownid">
                                            	<%
                                            		String zonearea=rs.getString(8);
                                            		int flag2=0;
                                            		String getZoneArea="select * from zone_area";
                                            		PreparedStatement st3=cn.prepareStatement(getZoneArea);
                                            		ResultSet rs3=st3.executeQuery();
                                            		while(rs3.next())
                                            		{
                                            			if(zonearea.equalsIgnoreCase(rs3.getString(3)))
                                            			{
                                            				flag2=1;
                                            			}
                                            			if(flag2==1)
                                            	...
                                            	%>
                                            		<option selected value="<%=rs3.getString(1) %>"><%=rs3.getString(3)%></option>
                                            	<%
                                            		flag2=0;
                                            			}
                                            			else
                                        ...
                                        <div id="divpassword" class="form-group">
                                            ...
                                            <input type="password" id="password" class="form-control underlined" name="password" value="<%=rs.getString(10) %>" onkeyup="passwordValidation()">
                                            <span id="passworderror" style="visibility:hidden"></span>
                                        </div>
                                        <div class="form-group" style="text-align:center;">
                                            <button type="submit" class="btn btn-primary" name="btn" value="update" style="width:40%;margin-top:15px;font-size:1.1rem;">Update</button>
                                        </div>
                                    </form>
      	  	...
      	  	var element2 = document.getElementById("zoner");
	   	  	...
	   	  	function selectZone(a){
					...
					success : function(data) {						
						var result = data.split('<body>')[1].split('<script>')[0];
						$('#zonearea').html(result);												
					}
				});
	  	  	}
